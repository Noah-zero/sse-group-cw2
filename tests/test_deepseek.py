...
# Import the Flask app defined in deepseek.py
...
# Provide a fake jwt.decode that always returns a fixed user info
def fake_jwt_decode(token, secret, algorithms):
    return {"user_id": 1}

# Define a DummySupabaseResponse and DummySupabaseTable
...
# Simulate client.table method by returning a DummySupabaseTable
...
def patch_deepseek_dependencies(monkeypatch):
    ...
    from deepseek import supabase_client
...
def test_start_chat_success(client_deepseek):
    ...
    response = client_deepseek.post("/start_chat", headers={"Authorization": token}, json=data)
...
def test_send_message_missing_token(client_deepseek):
    response = client_deepseek.post("/send_message", json={"message": "Hello", "chat_name": "Test Chat"})
    result = response.get_json()
